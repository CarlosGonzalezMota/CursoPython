print('*** Manejo de Sets ***')

# Crear un conjunto
mi_set = {1, 2, 3, 4, 5, 4} # Los sets NO permiten duplicidad de datos y tampoco respetan un orden
print(f'Mi set: {mi_set}')

# Agregar elementos al set
mi_set.add(6)
mi_set.add(7)
print(f'Mi set con elementos añadidos: {mi_set}')

# Intentamos agregar un elemento duplicado
mi_set.add(3) # Simplemente se ignora el elemento al estar duplicado
print(f'Mi set modificado: {mi_set}')

# Eliminar un elemento del conjunto
mi_set.remove(4)
print(f'Mi set eliminado: {mi_set}')

# Iterar los elementos del set
for elemento in mi_set:
    print(elemento, end=' ')

# Comprobar si existe un elemento en el set
print(f'\n¿Existe el valor 4 en el set? {4 in mi_set}')

# Obtener la longitud del set
print(f'Longitud del conjunto: {len(mi_set)}')